...
// The StatusTrackingFunc type is an adapter to allow the use of ordinary
// function as StatusTracking mutator.
type StatusTrackingFunc func(context.Context, *ent.StatusTrackingMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f StatusTrackingFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.StatusTrackingMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.StatusTrackingMutation", m)
	}
	return f(ctx, mv)
}
