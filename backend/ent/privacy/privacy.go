...
// The StatusTrackingQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type StatusTrackingQueryRuleFunc func(context.Context, *ent.StatusTrackingQuery) error

// EvalQuery return f(ctx, q).
func (f StatusTrackingQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.StatusTrackingQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.StatusTrackingQuery", q)
}

// The StatusTrackingMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type StatusTrackingMutationRuleFunc func(context.Context, *ent.StatusTrackingMutation) error

// EvalMutation calls f(ctx, m).
func (f StatusTrackingMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.StatusTrackingMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.StatusTrackingMutation", m)
}
