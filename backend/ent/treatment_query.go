...
type TreatmentQuery struct {
	...
	withStatusID  *StatusTrackingQuery
	withFKs       bool
...
func (tq *TreatmentQuery) QueryPatientID() *PatientQuery {
...
func (tq *TreatmentQuery) QueryDoctorID() *UserQuery {
...
func (tq *TreatmentQuery) QueryStatusID() *StatusTrackingQuery {
	query := &StatusTrackingQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TreatmentQuery) WithPatientID(opts ...func(*PatientQuery)) *TreatmentQuery {
	query := &PatientQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withPatientID = query
	return tq
}
...
func (tq *TreatmentQuery) WithDoctorID(opts ...func(*UserQuery)) *TreatmentQuery {
	query := &UserQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withDoctorID = query
	return tq
}
...
func (tq *TreatmentQuery) WithStatusID(opts ...func(*StatusTrackingQuery)) *TreatmentQuery {
	query := &StatusTrackingQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withStatusID = query
	return tq
}
...
//		GroupBy(treatment.FieldDateAppoint).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Treatment.Query().
//		Select(treatment.FieldDateAppoint).
//		Scan(ctx, &v)
//
...
func (tq *TreatmentQuery) sqlAll(ctx context.Context) ([]*Treatment, error) {
	var (
		...
		_spec       = tq.querySpec()
		loadedTypes = [3]bool{
			tq.withPatientID != nil,
			tq.withDoctorID != nil,
			tq.withStatusID != nil,
		}
	)
	if tq.withPatientID != nil || tq.withDoctorID != nil || tq.withStatusID != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := tq.withPatientID; query != nil {
	...
	if query := tq.withDoctorID; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Treatment)
		for i := range nodes {
	...
	if query := tq.withStatusID; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Treatment)
		for i := range nodes {
			if fk := nodes[i].status_tracking_status_id; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(statustracking.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "status_tracking_status_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.StatusID = n
			}
		}
	}
