...
export function EntPatientEdgesFromJSONTyped(json: any, ignoreDiscriminator: boolean): EntPatientEdges {
    ...
    return {
        ...
        'patientID': !exists(json, 'patientID') ? undefined : ((json['patientID'] as Array<any>).map(EntTreatmentFromJSON)),
    };
}
...
export function EntPatientEdgesToJSON(value?: EntPatientEdges | null): any {
    ...
    return {
        ...
        'patientID': value.patientID === undefined ? undefined : ((value.patientID as Array<any>).map(EntTreatmentToJSON)),
    };
}
